...
WebApp.connectHandlers.use('/standalone-viz/visualizationn.html', (req, res, next) => {
  try {
...
Meteor.methods({
  runPythonVisualization({ variable1, variable2 }) {
    ...
    return { 
      success: true, 
      outputPath: `/standalone-viz/visualizationn.html?var1=${encodeURIComponent(variable1)}&var2=${encodeURIComponent(variable2)}&t=${Date.now()}` 
    };
  },
});