...
from support_files.dataLoader_n import load_new_data
...
pcoa_results, demographic_data = load_new_data()
...
pcoa_base = pcoa_results.samples.copy()
...
if hasattr(pcoa_results, 'proportion_explained'):
    original_explained = pcoa_results.proportion_explained.values
else:
...
all_explained = np.concatenate([
    original_explained,
    np.zeros(n_total_axes - len(original_explained))  # Zeros for metadata axes
])
